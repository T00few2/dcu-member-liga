...
function RegisterContent() {
  ...
  // Prefill name from Google Auth if available
  useEffect(() => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Get Firebase ID Token
      // const idToken = await user.getIdToken(); // already declared above
      ...
      const res = await fetch(`${apiUrl}/signup`, {
        ...
        body: JSON.stringify({ 
            ...
            name, 
            zwiftId,
            uid: user.uid // Redundant if token verified, but useful for now
        }),
      });
      ...
      // Save eLicense locally for Strava linking helper
  ...
  return (
    <div className="max-w-md mx-auto mt-10 p-6 bg-white rounded-lg shadow-md">
      <h1 className="text-2xl font-bold mb-6 text-slate-800">League Registration</h1>
      ...
      {status === 'success' ? (
        <div className="bg-green-50 text-green-700 p-4 rounded-md">
          ...
          <button 
            onClick={() => { 
                setStatus('idle'); 
                // Don't clear name if it came from Auth
