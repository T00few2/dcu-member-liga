...
export default function ProfilePage() {
    ...
    // Redirect unauthenticated / unregistered users
    useEffect(() => {
        if (!authLoading && (!user || !isRegistered)) {
            router.replace('/');
        }
    }, [authLoading, user, isRegistered, router]);
    ...
    return (
        <div className="max-w-3xl mx-auto px-4 py-10 pb-24">
            <h1 className="text-3xl font-bold mb-1 text-foreground">Min Profil</h1>
            <p className="text-muted-foreground mb-8">{profile.name}</p>

            {/* Tabs */}
            <div className="flex gap-2 border-b border-border mb-8">
                {(['kategori', 'oversigt'] as Tab[]).map(t => (
                    <button
                        key={t}
                        onClick={() => setTab(t)}
                        className={`pb-2 px-4 font-medium capitalize transition ${
                            tab === t
                                ? 'text-primary border-b-2 border-primary'
                                : 'text-muted-foreground hover:text-foreground'
                        }`}
                    >
                        {t === 'kategori' ? 'Kategori' : 'Oversigt'}
                    </button>
                ))}
            </div>
            ...
            {tab === 'oversigt' && (
                <div className="space-y-4">
                    <div className="bg-card border border-border rounded-lg p-6 shadow-sm space-y-3 text-sm">
                        {[
                            { label: 'Navn',      value: profile.name },
                            { label: 'eLicens',   value: profile.eLicense },
                            { label: 'Zwift ID',  value: profile.zwiftId },
                            { label: 'Kategori',  value: lc ? `${catMeta(lc.category)?.gem} ${lc.category}` : '—' },
                        ].map(row => (
                            <div key={row.label} className="flex justify-between gap-4 border-b border-border/50 pb-2 last:border-0 last:pb-0">
