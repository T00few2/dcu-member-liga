                    ...
                    <div key={group.id} className="p-4 bg-muted/20 rounded-lg border border-border space-y-3">
                        <div className="flex gap-2 items-start">
                            <div className="flex-1">
                                ...
                                <input type="text" value={group.name} onChange={e => onUpdateRaceGroup(groupIdx, 'name', e.target.value)} className="w-full p-2 border border-input rounded bg-background text-foreground text-sm" placeholder="e.g. High end" />
                            </div>
                            <div className="flex-1">
                                <label className="text-[10px] text-muted-foreground font-bold uppercase block mb-1">Zwift Event ID</label>
                                <input type="text" value={group.eventId} onChange={e => onUpdateRaceGroup(groupIdx, 'eventId', e.target.value)} className="w-full p-2 border border-input rounded bg-background text-foreground text-sm" placeholder="e.g. 12345" />
                            </div>
                            <div className="flex-1">
                                <label className="text-[10px] text-muted-foreground font-bold uppercase block mb-1">Secret (Opt)</label>
                                <input type="text" value={group.eventSecret || ''} onChange={e => onUpdateRaceGroup(groupIdx, 'eventSecret', e.target.value)} className="w-full p-2 border border-input rounded bg-background text-foreground text-sm" placeholder="Secret" />
                            </div>
                            <button type="button" onClick={() => onRemoveRaceGroup(groupIdx)} className="text-red-500 hover:text-red-700 px-2 pt-6">✕</button>
                        </div>

                        <div className="flex gap-2 items-start">
                            <div className="w-20">
                                <label className="text-[10px] text-muted-foreground font-bold uppercase block mb-1">Laps</label>
                                <input type="number" value={groupMaxLaps} onChange={e => onUpdateRaceGroup(groupIdx, 'laps', parseInt(e.target.value) || 1)} className="w-full p-2 border border-input rounded bg-background text-foreground text-sm" min="1" />
                            </div>
                            <div className="w-40">
                                <label className="text-[10px] text-muted-foreground font-bold uppercase block mb-1">Segments Used For</label>
                                <select value={group.segmentType || 'sprint'} onChange={e => onUpdateRaceGroup(groupIdx, 'segmentType', e.target.value as 'sprint' | 'split')} className="w-full p-2 border border-input rounded bg-background text-foreground text-sm">
                        ...
                        <div className="space-y-2">
                                ...
                                return (
                                    <div key={catIdx} className="p-2 bg-background rounded border border-border space-y-2">
                                        <div className="flex gap-2 items-center">
                                            ...
                                            <div className="w-20">
                                                <label className="text-[10px] text-muted-foreground font-bold uppercase block mb-1">Laps</label>
                                                <input type="number" value={cat.laps || groupMaxLaps} onChange={e => onUpdateGroupCategory(groupIdx, catIdx, 'laps', parseInt(e.target.value) || 1)} className="w-full p-2 border border-input rounded bg-background text-foreground text-sm" min="1" />
                                            </div>
                                            <button type="button" onClick={() => onRemoveGroupCategory(groupIdx, catIdx)} className="text-red-500 hover:text-red-700 px-2 pt-5">✕</button>
                                        </div>
                                        <CollapsibleSegmentPicker
                            ...
                            <button type="button" onClick={() => onAddGroupCategory(groupIdx)} className="text-xs text-primary hover:text-primary/80 font-medium">+ Add Category</button>
                        </div>
                    </div>
                );
            })}
            <button type="button" onClick={onAddRaceGroup} className="text-sm text-primary hover:text-primary/80 font-medium">+ Add Group</button>
        </div>
    );
}
