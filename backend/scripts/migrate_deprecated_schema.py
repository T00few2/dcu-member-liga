...
Usage (from the backend/ directory):
    python scripts/migrate_deprecated_schema.py [--dry-run]
...
def migrate(dry_run: bool = False) -> None:
    ...
    logger.info("Total user documents: %s", len(docs))
    ...
    logger.info("Documents requiring migration: %s", len(to_migrate))
    ...
    if dry_run:
        for doc in to_migrate:
            data = doc.to_dict() or {}
            old_signal = "registrationComplete=True" if data.get("registrationComplete") else "verified=True"
            logger.info("  [DRY-RUN] Would migrate %s (%s)", doc.id, old_signal)
        return
    ...
    for doc in to_migrate:
        ...
        count += 1
        logger.info("  Migrating %s (%s) → registration.status=complete", doc.id, old_signal)
    ...
    logger.info("Migration complete: %s documents updated.", count)
